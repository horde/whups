...
Horde_Registry::appInit('whups', [
    'permission' => ['whups:admin', Horde_Perms::EDIT],
]);
    ...
    case 'whups_form_admin_edittypesteptwo':
        ...
        if ($form->validate($vars)) {
            try {
                ...
                $notification->push(
                    sprintf(
                        _("The type \"%s\" has been modified."),
                        $vars->get('name')
                    ),
                    'horde.success'
                );
    ...
    case 'whups_form_admin_addqueue':
        ...
        if ($form->validate($vars)) {
            try {
                ...
                $notification->push(
                    sprintf(
                        _("The queue \"%s\" has been created."),
                        $vars->get('name')
                    ),
                    'horde.success'
                );
    ...
    case 'whups_form_admin_addstate':
        ...
        if ($form->validate($vars)) {
            try {
                ...
                $notification->push(
                    sprintf(
                        _("The state \"%s\" has been added to %s."),
                        $vars->get('name'),
                        $typename
                    ),
                    'horde.success'
            ...
            $vars = new Horde_Variables(['type' => $vars->get('type')]);
            _editStateForms();
        } else {
            _open();
            $form->renderActive($renderer, $vars, $adminurl, 'post');
    ...
    case 'whups_form_admin_addpriority':
        ...
        if ($form->validate($vars)) {
            try {
                ...
                $notification->push(
                    sprintf(
                        _("The priority \"%s\" has been added to %s."),
                        $vars->get('name'),
                        $typename
                    ),
                    'horde.success'
    ...
    case 'whups_form_admin_adduser':
        ...
        if ($form->validate($vars)) {
            ...
            try {
                ...
                $notification->push(
                    sprintf(
                        _("%s added to those responsible for \"%s\""),
                        $userinfo,
                        $info['name']
                    ),
                    'horde.success'
                ...
                $notification->push(
                    sprintf(
                        _("There was an error adding \"%s\" to the responsible list for \"%s\":"),
        ...
        $form1->renderActive($renderer, $vars, $adminurl, 'post');
        echo '<br />';
        $vars = new Horde_Variables(['queue' => $vars->get('queue')]);
        $form->renderActive($renderer, $vars, $adminurl, 'post');
        break;
    ...
    case 'whups_form_admin_edituser':
        ...
        if ($form->validate($vars)) {
            ...
            try {
                ...
                $notification->push(
                    sprintf(
                        _("\"%s\" is no longer among those responsible for \"%s\""),
                        Whups::formatUser($vars->get('user')),
                        $info['name']
                    ),
                    'horde.success'
                ...
                $notification->push(
                    sprintf(
                        _("There was an error removing \"%s\" from the responsible list for \"%s\":"),
                        Whups::formatUser($vars->get('user')),
                        $info['name']
                    )
                    . ' ' . $e->getMessage(),
    ...
    case 'whups_form_admin_addversion':
        ...
        if ($form->validate($vars)) {
            try {
                ...
                $notification->push(
                    sprintf(
                        _("The version \"%s\" has been added to %s."),
                        $vars->get('name'),
                        $queuename
                    ),
                    'horde.success'
    ...
    case 'whups_form_admin_addattribute_reload':
        ...
        if ($vars->get('formname') == 'whups_form_admin_addattribute'
            ...
            try {
                ...
                $notification->push(
                    sprintf(
                        _("The attribute \"%s\" has been added to %s."),
                        $vars->get('attribute_name'),
                        $typename
                    ),
                    'horde.success'
        ...
        if ($vars->get('formname') == 'whups_form_admin_editattributesteptwo'
            ...
            try {
                $whups_driver->updateAttributeDesc(
                    ...
                    $info['attribute_type'],
                    !empty($info['attribute_params']) ? $info['attribute_params'] : [],
    ...
    case 'whups_form_admin_addreply':
        ...
        if ($form->validate($vars)) {
            try {
                ...
                $notification->push(
                    sprintf(
                        _("The form reply \"%s\" has been added to %s."),
                        $vars->get('reply_name'),
                        $typename
                    ),
                    'horde.success'
