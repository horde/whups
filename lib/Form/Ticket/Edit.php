...
class Whups_Form_Ticket_Edit extends Horde_Form
{
    public function __construct($vars, $ticket, $title = '')
    {
        ...
        $fields = array_merge($fields, ['state', 'priority', 'due']);
        try {
        ...
        $fields = array_merge($fields, ['owner', 'attachments', 'comment']);

        $grouped_fields = [$fields];
        $grouped_hook = false;
        try {
            $grouped_fields = $GLOBALS['injector']
                ->getInstance('Horde_Core_Hooks')
                ->callHook('group_fields', 'whups', [$ticket->get('type'), $fields]);
            $grouped_hook = true;
                ...
                switch ($field) {
                    ...
                    case 'priority':
                        $this->addVariable(
                            ...
                            false,
                            null,
                            [$whups_driver->getPriorities($type)]
                        );
                        break;
                    ...
                    case 'comment':
                        ...
                        if ($mygroups) {
                            ...
                            asort($grouplist);
                            $grouplist = [0 => _("This comment is visible to everyone")] + $grouplist;
                            $this->addVariable(
