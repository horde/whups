...
class Whups_Form_Query_PropertyCriterion extends Horde_Form
{
    public function __construct($vars)
    {
        ...
        $this->addVariable(
            ...
            false,
            null,
            [$whups_driver->getAllTypes(), _("Any")]
        );
        ...
        if (count($queues)) {
            ...
            if ($vars->get('queue')) {
                $this->addVariable(
                    ...
                    false,
                    null,
                    [$whups_driver->getVersions($vars->get('queue')), _("Any")]
                );
            }
        }
    ...
    public function execute($vars = null)
    {
        ...
        if ($priority) {
            $GLOBALS['whups_query']->insertCriterion(
                $path,
                Whups_Query::CRITERION_PRIORITY,
                null,
                Whups_Query::OPERATOR_EQUAL,
                $priority
            );
