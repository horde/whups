...
if (($vars->get('formname') || $vars->get('summary') || $vars->get('states')
    ...
    if ($vars->get('submitbutton') == _("Save as Query")) {
        ...
        foreach (['ticket_timestamp', 'date_updated', 'date_resolved', 'date_assigned', 'date_due'] as $date_field) {
        ...
        if (!empty($info['ticket_timestamp']['from'])) {
            $whups_query->insertCriterion(
                $path,
                Whups_Query::CRITERION_TIMESTAMP,
                null,
                Whups_Query::OPERATOR_GREATER,
                $info['ticket_timestamp']['from']
            );
        }
        if (!empty($info['ticket_timestamp']['to'])) {
            $whups_query->insertCriterion(
                ...
                $info['ticket_timestamp']['to']
            );
        }
        if (!empty($info['date_updated']['from'])) {
            $whups_query->insertCriterion(
                $path,
                Whups_Query::CRITERION_UPDATED,
                null,
                Whups_Query::OPERATOR_GREATER,
                $info['date_updated']['from']
            );
        }
        if (!empty($info['date_updated']['to'])) {
            $whups_query->insertCriterion(
                ...
                null,
                Whups_Query::OPERATOR_LESS,
                $info['date_updated']['to']
            );
        }
        if (!empty($info['date_resolved']['from'])) {
            $whups_query->insertCriterion(
                $path,
                Whups_Query::CRITERION_RESOLVED,
                null,
                Whups_Query::OPERATOR_GREATER,
                $info['date_resolved']['from']
            );
        }
        if (!empty($info['date_resolved']['to'])) {
            $whups_query->insertCriterion(
                ...
                null,
                Whups_Query::OPERATOR_LESS,
                $info['date_resolved']['to']
            );
        }
        if (!empty($info['date_assigned']['from'])) {
            $whups_query->insertCriterion(
                $path,
                Whups_Query::CRITERION_ASSIGNED,
                null,
                Whups_Query::OPERATOR_GREATER,
                $info['date_assigned']['from']
            );
        }
        if (!empty($info['date_assigned']['to'])) {
            $whups_query->insertCriterion(
                ...
                null,
                Whups_Query::OPERATOR_LESS,
                $info['date_assigned']['to']
            );
        }
        if (!empty($info['date_due']['from'])) {
            $whups_query->insertCriterion(
                $path,
                Whups_Query::CRITERION_DUE,
                null,
                Whups_Query::OPERATOR_GREATER,
                $info['date_due']['from']
            );
...
$myqueries = new Whups_View_SavedQueries(
    ['title' => $GLOBALS['registry']->getAuth() ? _("My Queries") : _("Public Queries"),
