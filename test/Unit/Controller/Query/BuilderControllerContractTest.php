<?php

declare(strict_types=1);

namespace Horde\Whups\Test\Unit\Controller\Query;

use PHPUnit\Framework\TestCase;

/**
 * Contract tests for query/index.php → BuilderController
 *
 * Entrypoint: query/index.php
 * Purpose: List saved queries and provide query management/editing interface
 * Route: /whups/query
 * @coversNothing
 */
class BuilderControllerContractTest extends TestCase
{
    public function testListsSavedQueries(): void
    {
        $this->markTestIncomplete('Awaiting controller conversion from query/index.php');
    }

    public function testDisplaysQueryBuilder(): void
    {
        $this->markTestIncomplete('Awaiting controller conversion from query/index.php');
    }

    public function testRequiresAuthentication(): void
    {
        $this->markTestIncomplete('Awaiting controller conversion from query/index.php');
    }

    public function testReturnsHtmlResponse(): void
    {
        $this->markTestIncomplete('Awaiting controller conversion from query/index.php');
    }
}
